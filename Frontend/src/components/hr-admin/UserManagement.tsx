...
const UserManagement: React.FC = () => {
  ...
  const [isEditNewDepartment, setIsEditNewDepartment] = useState<boolean>(false);
  const [editDepartmentName, setEditDepartmentName] = useState<string>('');
  const [editEmailError, setEditEmailError] = useState<string>('');
  const [availableEditManagers, setAvailableEditManagers] = useState<User[]>([]);
  ...
  useEffect(() => {
    if (showEditUser && editUser.department && editUser.role === 'employee') {
      const departmentManagers = managers.filter(manager =>
        manager.department === editUser.department
      );
      setAvailableEditManagers(departmentManagers);
    } else {
      setAvailableEditManagers([]);
    }
  }, [showEditUser, editUser.department, editUser.role, managers]);
  ...
  const handleEditEmailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setEditEmailError('');
    if (email && !email.includes('@obu.edu.et')) {
      setEditEmailError(t('user_management.errors.email_domain'));
    }
  };
  ...
  const handleEditDepartmentNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleUpdateUser = async (e: React.FormEvent): Promise<void> => {
    e.preventDefault();
    if (!editingUserId) {
      setError(t('user_management.errors.update_failed', { defaultValue: 'Failed to update user' }));
      return;
    }
    ...
    try {
      ...
      if (!editUser.name.trim() || !editUser.email.trim() || !editUser.department.trim()) {
        setError(t('user_management.errors.required_fields'));
        return;
      }

      if (!editUser.email.includes('@obu.edu.et')) {
        setEditEmailError(t('user_management.errors.email_domain'));
        return;
      }
      ...
      if (duplicateEmailUser) {
        setEditEmailError(t('user_management.errors.email_used', { name: duplicateEmailUser.name }));
        return;
      }

      const finalDepartment = isEditNewDepartment ? editDepartmentName : editUser.department;
      const payload: Omit<Partial<User>, 'managerId'> & { managerId?: number | null } = {
      ...
      if (editUser.role === 'employee') {
        payload.managerId = editUser.managerId ? parseInt(editUser.managerId, 10) : null;
      } else {
      ...
      const response = await apiService.updateUser(editingUserId, payload);
      if (!response.success) {
        setError(response.message || t('user_management.errors.update_failed', { defaultValue: 'Failed to update user' }));
        return;
      }

      setSuccess(
        t('user_management.messages.updated', {
          defaultValue: 'User "{{name}}" updated successfully!',
          name: editUser.name
        })
      );
      await loadUsers();
      ...
      setError(updateError.message || t('user_management.errors.update_failed', { defaultValue: 'Failed to update user' }));
    } finally {
      setEditSubmitting(false);
    }
  };
 ...
 const generatePassword = (): void => {
  ...
  return (
    <div className="user-management">
      ...
      {showEditUser && (
        <div className="modal-overlay">
          <div className="modal-content" role="dialog" aria-modal="true" aria-label={t('user_management.modal.edit_title', { defaultValue: 'Edit User' })}>
            <div className="modal-header">
              <h3>{t('user_management.modal.edit_title', { defaultValue: 'Edit User' })}</h3>
              <button
                className="close-btn"
            ...
            <form onSubmit={handleUpdateUser} className="user-form">
              <div className="form-section">
                ...
                <div className="form-row">
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      required
                      placeholder={t('user_management.placeholders.email')}
                      className={editEmailError ? 'error' : (editUser.email ? 'success' : '')}
                    />
                    {editEmailError && (
                      <small className="error-text">{editEmailError}</small>
                    )}
                    {!editEmailError && editUser.email && editUser.email.includes('@obu.edu.et') && (
                      <small className="success-text">{t('user_management.help.valid_email')}</small>
                    )}
                  </div>
                </div>
                ...
                <div className="form-row">
                  ...
                  <div className="form-group">
                    ...
                    <select
                      name="department"
                      value={isEditNewDepartment ? 'new' : editUser.department}
              ...
              <div className="form-section">
                ...
                {editUser.role === 'employee' && editUser.department && (
                  <div className="form-group">
                    <label>{t('user_management.fields.manager')}</label>
                    <select
                      name="managerId"
                    ...
                    <small className="help-text">
                      {availableEditManagers.length === 0 && editUser.department
                        ? t('user_management.help.no_managers', { department: editUser.department })
                        : t('user_management.help.select_manager')
                      }
                    </small>
                  </div>
              ...
              <div className="form-section">
                <h4>{t('user_management.modal.security')}</h4>
                <div className="form-group">
                  <label>{t('user_management.fields.new_password', { defaultValue: 'New Password (Optional)' })}</label>
                  <input
                    type="text"
                    name="password"
                  ...
                  />
                  <small className="help-text">
                    {t('user_management.help.password_optional', { defaultValue: 'Leave blank to keep current password.' })}
                  </small>
                </div>
              </div>
              ...
              <div className="modal-actions">
                ...
                >
                  {editSubmitting
                    ? t('user_management.actions.updating', { defaultValue: 'Updating...' })
                    : t('user_management.actions.update', { defaultValue: 'Update User' })}
                </button>
                <button
                  type="button"
                  className="cancel-btn"
                    ...
                    <td>
                      <div className="action-buttons">
                        <button
                          ...
                          title={t('user_management.actions.edit', { defaultValue: 'Edit' })}
                        >
                          {t('user_management.actions.edit', { defaultValue: 'Edit' })}
                        </button>
                        <button 
